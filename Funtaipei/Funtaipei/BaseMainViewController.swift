...
class BaseMainViewController: UIViewController, CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    let myTableViewBackgroundColor = UIColor(red: 1, green: 1, blue: 0.9, alpha: 1)
    ...
    func addTable(_ file :String?) {
        ...
        self.myTableView.backgroundColor = myTableViewBackgroundColor
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.backgroundColor = myTableViewBackgroundColor
        ...
        if let myLabel = cell!.textLabel {
            ...
            } else if let title = thisData["Name"] as? String {
                myLabel.text = title as String
    ...
    func reloadAPIData() {
        ...
        if locationAuth != nil && locationAuth! {
            ...
            if userLocation.distance(from: recordLocation) > self.limitDistance {
                ...
                let runLimit : Int!
                if self.apiDataForDistance.count < self.limitNumber {
                    runLimit = self.apiDataForDistance.count - 1
                } else {
                    runLimit = self.limitNumber
                }
                
                for index in 0...runLimit {
